...
const TopChartsMobile = ({ topChartsData }) => {
  const topCharts = topChartsData.slice(0, 10);
  ...
  return (
    ...
    >
      {topCharts.map((chart) => (
        <SwiperSlide key={chart.key} className="w-[220px] sm:w-[250px]">
          <div className="flex-col p-3.5 gap-y-3 bg-altDark flex rounded-[20px]">
            ...
            <div className="flex flex-col flex-1 justify-start min-w-0">
              <Link to="/songs/2" className="truncate">
                {chart.title}
              </Link>
