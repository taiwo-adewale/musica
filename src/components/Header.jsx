...
const Header = ({setIsToggled, isAlbum}) => {
  return (
    <header id='header' className={`fixed left-0 right-0 top-0 z-[4] transition-all duration-150 ${ isAlbum ? '' : 'bg-bgDark' }`}>
      <div className="flex items-center h-[70px] px-[28px] gap-x-9">
        <img src={images.logo} alt="logo" className='w-[34px] h-[34px]' />

        <div className={`hidden sm:flex items-center gap-x-2 px-3 py-1 rounded-lg ${ isAlbum ? '' : 'bg-altDark' }`}>
          <FiSearch className='w-5 h-5 stroke-[rgba(255,255,255,0.25)]' />

          <input type="text" placeholder='Search' className='w-[300px] bg-transparent text-sm p-2 focus:outline-none placeholder:text-[rgba(255,255,255,0.25)] ' />
        </div>
